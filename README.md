...
- **Sample datasets**: variable sets injected at compile time, overriding template defaults exactly as a `--variables` file would with the CLI compilers. The "Weekly forecast summary" sample shows data-driven generation: its placeholders reference the forecast dataset by name (`dataSource: "forecast"`), the compiler renders it once as a REFERENCE DATA table above the template that the model uses but never outputs, and swapping datasets changes the generated story. Requires its-compiler-js 1.3.0 and its-compiler 1.2.0; until those reach the registries, both engines resolve the compilers from git (see package.json and server/pyproject.toml).
