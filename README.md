...
- **Sample datasets**: variable sets injected at compile time, overriding template defaults exactly as a `--variables` file would with the CLI compilers. The "Weekly forecast summary" sample shows data-driven generation: a forecast dataset is injected into the placeholders' user prompts, so the model reads the data and summarises its trends without reproducing it in the output, and swapping datasets changes the generated story.
